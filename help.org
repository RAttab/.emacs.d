...
*** p/xdutacf -> print as hex/signed/unsigned/bin/adress/char/float
*** dump/append binary/ihex memory <filename> <start_addr> <end_addr>
*** layout src | asm | regs | split -> Show source code, asm, or both
*** focus src | asm -> Switch to window and scroll with arrows.
** Logging
*** set logging file FILE
*** set logging [on|off]
** Utils
*** set $var_name=value -> Set that value for later use
*** p $var_name -> prints a saved value (works for registers and prev cmd).
** Emacs
